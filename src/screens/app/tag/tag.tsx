...
import {Button, Container, Spinner, View, Text, H3} from 'native-base';
...
type ScreenNavigationProp = StackNavigationProp<AppStackParamList, 'Tag'>;
type ScreenRouteProp = RouteProp<AppStackParamList, 'Tag'>;
...
const TagScreen: React.FC<IProps> = (props) => {
    const {tagId, tagName} = props.route.params;
    ...
    const getProducts = (currentPage: number) => {
        ...
        TagService.getProductsInTag(tagId, page, elementsPerPage, sortBy)
            .then(({data}) => {
            ...
            .catch((error) => {
                console.log('ERROR LOADING TAG PRODUCTS:', error);
            })
            .finally(() => {
                ...
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View style={{marginHorizontal: 20}}>
                            <Text>
                                Mostrando productos del grupo '
                                <Text style={componentStyles.bold}>{tagName}</Text>'
                            </Text>
                        </View>
                    }
                    ListFooterComponent={
                                ...
                                !loadingProducts && (
                                    <>
                                        <AlertText style={{marginTop: 15}}>
                                            No se encontraron productos dentro del grupo
                                            seleccionado.
                                        </AlertText>
                                        <Button
                                            transparent
